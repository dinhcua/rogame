      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2 ml-4">
          {sharedSave.isDownloaded && onRestore ? (
            // Show only Restore button with blue color when downloaded
            <button
              onClick={() => onRestore(sharedSave)}
              disabled={isRestoring}
              className="px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-700 transition-all duration-200 font-medium text-sm text-white flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isRestoring ? (
                <>
